...
import { Button, Icon, Input, Switch, ListItem } from 'react-native-elements'
...
const SettingsView = ({ route, navigation }) => {
  ...
  const renderPluginSettings = (category: string) => {
    return settingsService.getPluginSettings(category).map((setting: any, i: number) => {
      ...
      <ListItem key={setting.name} containerStyle={styles.listItemContainer} bottomDivider>
        <ListItem.Content>
          ...
          <DropDownPicker
            ...
            setItems={(items: any) => setItems(items)}
            listItemLabelStyle={{
              fontSize: 18
            }}
            style={{
