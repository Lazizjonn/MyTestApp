...
const TSMapView = (props) => {
  ...
  const [geofencesHit, setGeofencesHit] = React.useState([]);
  const [geofencesHitEvents, setGeofenceHitEvents] = React.useState([]);
  ...
  const [lastMotionChangeEvent, setLastMotionChangeEvent] = React.useState(null);
  const [geofences, setGeofences] = React.useState([]);
  const [polygonGeofences, setPolygonGeofences] = React.useState([]);
  const [geofenceEvent, setGeofenceEvent] = React.useState(null);
  const [geofencesChangeEvent, setGeofencesChangeEvent] = React.useState(null);
  const [enabled, setEnabled] = React.useState(false);
  /// Creating a polygon geofence
  const [isCreatingPolygon, setIsCreatingPolygon] = React.useState(false);
  const [createPolygonGeofenceCoordinates, setCreatePolygonGeofenceCoordinates] = React.useState([]);
  ...
  const onGeofencesChange = () => {
    ...
    let geofencesOn = geofences.filter((geofence) => {
      return off.indexOf(geofence.identifier) < 0;
    });
    let polygonsOn = polygonGeofences.filter((geofence) => {
      return (off.indexOf(geofence.identifier) < 0);
    });

    // Add new "on" geofences.
    on.forEach((geofence) => {
      const circularGeofenceMarker = geofencesOn.find((m) => { return m.identifier === geofence.identifier; });
      if (!circularGeofenceMarker) {
        geofencesOn.push(createGeofenceMarker(geofence));
      }
      if (geofence.vertices.length > 0) {
        const polygonGeofenceMarker = polygonsOn.find((m) => {
  ...
  const renderGeofencesHitEvents = () => {
    return geofencesHitEvents.map((event) => {
  ...
  const createPolygonGeofenceMarker = (geofence) => {
    return {
      identifier: geofence.identifier,
  ...
  const renderCreatePolygonGeofenceVertices = () => {
    var index = 0;
    return createPolygonGeofenceCoordinates.map((coordinate) => {
      return (<Marker
