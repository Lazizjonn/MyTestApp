...
const HomeView = ({route, navigation}) => {
  ...
  const [motionActivityEvent, setMotionActivityEvent] = React.useState();
  const [testClicks, setTestClicks] = React.useState(0);
  const [clickBufferTimeout, setClickBufferTimeout] = React.useState(0);
  ...
  React.useEffect(() => {
    ...
    const notificationActionSubscriber = BackgroundGeolocation.onNotificationAction((button) => {
      console.log('[onNotificationAction]', button);
    });

    const heartbeatSubscriber = BackgroundGeolocation.onHeartbeat(async (event) => {
  ...
  const initBackgroundGeolocation = async () => {
    ...
    const state = await BackgroundGeolocation.ready({
      ...
      backgroundPermissionRationale: {
        title: "Allow CityMaaSPlatform to access this device's location even when closed or not in use.",
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FABMenu navigation ={navigation} onResetOdometer={(location) => setOdometer(location.odometer)}/>
