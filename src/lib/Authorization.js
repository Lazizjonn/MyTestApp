...
import BackgroundGeolocation, { TransistorAuthorizationToken, HttpEvent } from "./react-native-background-geolocation"
import ENV from "./ENV";

let onHttpSubscription = null;
/**
...
*/
async function goHome(navigation) {
...
*/
export async function registerTransistorAuthorizationListener(navigation) {
  ...
  onHttpSubscription = BackgroundGeolocation.onHttp(async (event) => {
    switch (event.status) {
      case 403:
      case 406:
        await BackgroundGeolocation.destroyTransistorAuthorizationToken(ENV.TRACKER_HOST);
